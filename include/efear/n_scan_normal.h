...
public:
  ...
  n_scan_normal_reg(const cost_metric& cost, loss_type loss = Huber, double loss_limit = exp(-16), const weightoption opt = weightoption::Combined_weights);

  bool Register(std::vector<MapNormalPtr>& scans, std::vector<Eigen::Isometry3d>& Tsrc, std::vector<Matrix6d>& reg_cov, bool soft_constraints = false);
  ...
  bool GetCost(std::vector<MapNormalPtr>& scans, std::vector<Eigen::Isometry3d>& Tsrc, double& score, std::vector<double>& residuals);

  void GetSurface(std::vector<MapNormalPtr>& scans, std::vector<Eigen::Isometry3d>& Tsrc, std::vector<Matrix6d> &reg_cov, bool soft_constraints, Eigen::MatrixXd& surface, double res, int width);
  ...
  void AddScanPairCost(MapNormalPtr& target_local, MapNormalPtr& src_local, 
...
private:
  ...
  bool BuildOptimizationProblem(std::vector<MapNormalPtr>& scans, 
  const Eigen::MatrixXd& cov, //= Eigen::Matrix<double,3,3>::Identity(), 
  Vector6d& guess,
  ...
  const double max_itr = 8, min_itr = 1;
  const bool efficient_implementation = true;

  bool time_continuous_ = false;
  std::vector<double> vel_parameters_;
  bool ccw_ = false;
  std::vector<std::tuple<Eigen::Vector3d,Eigen::Vector3d,double,int> > vis_residuals;
...
protected:
  ...
  template <typename T>
  static Eigen::Matrix<T, 3, 1> GetTransMatrix(const T*  par) {
    Eigen::Matrix<T, 3, 1> trans {par[0], par[1], par[2]};
    return trans;
  }
  // template <typename T>
  ...
  // }
  template <typename T>
  static Eigen::Matrix<T, 3, 3> GetRotMatrix(const T* par){

    const T roll = par[3];
    const T pitch = par[4];
    const T yaw = par[5];

    Eigen::Matrix<T, 3, 3> R, R_x, R_y, R_z;
    R_x << (T)1,(T)0,(T)0,
    (T)0, ceres::cos(yaw),-ceres::sin(yaw),
    (T)0, ceres::sin(yaw),ceres::cos(yaw);
    R_y << 
    ceres::cos(pitch), (T)0, ceres::sin(pitch),
    (T)0,(T)1,(T)0,
    -ceres::sin(pitch),(T)0,ceres::cos(pitch);
    R_z<<
    ceres::cos(roll), -ceres::sin(roll),(T)0,
    ceres::sin(roll), ceres::cos(roll), (T)0,
    (T)0,(T)0,(T)1;
    R = R_z * R_y * R_x;
  ...
  template <typename T>
  static Eigen::Matrix<T, 3, 1> GetScaledTransMatrix(const T*  par, const T scale) {
    Eigen::Matrix<T, 3, 1> trans(scale*par[0], scale*par[1],scale*par[2]);
    return trans;
  }
};
...
//   P2LCost (const Eigen::Vector3d& target_mean, const Eigen::Vector3d& target_normal, const Eigen::Vector3d& src_mean) :
...
//       const Eigen::Vector3d& target_mean, const Eigen::Vector3d& target_normal, const Eigen::Vector3d& src_mean) {
//     return new ceres::AutoDiffCostFunction<P2LCost ,1, 3, 3>(new P2LCost (target_mean, target_normal, src_mean));
//   }
//   EIGEN_MAKE_ALIGNED_OPERATOR_NEW
//   private:

//   const Eigen::Vector3d tar_mean_;
//   const Eigen::Vector3d tar_normal_;
//   const Eigen::Vector3d src_mean_;
// };
...
public:
  ...
  P2LEfficientCost (const Eigen::Vector3d& transformed_mean_tar, const Eigen::Vector3d& transformed_normal_tar, const Eigen::Vector3d& src_mean) :
  ...
  bool operator()(const T*  Tb,
    ...
    const Eigen::Matrix<T,3,1> trans_mat_src = GetTransMatrix(Tb);
    const Eigen::Matrix<T,3,3> rot_mat_src = GetRotMatrix(Tb);

    const Eigen::Matrix<T,3,1> transformed_mean_src = (rot_mat_src * (src_mean_).cast<T>()) + trans_mat_src;
    const Eigen::Matrix<T,3,1> v = transformed_mean_src - transformed_mean_tar_.cast<T>();
    const Eigen::Matrix<T,3,1> n = transformed_normal_tar_.cast<T>();
    residuals_ptr[0] = v.dot(n);
    return true;
  }

  static ceres::CostFunction* Create(
      const Eigen::Vector3d& transformed_mean_tar, const Eigen::Vector3d& transformed_normal_tar, const Eigen::Vector3d& src_mean) {
    return new ceres::AutoDiffCostFunction<P2LEfficientCost, 1, 6>(new P2LEfficientCost (transformed_mean_tar, transformed_normal_tar, src_mean));
  }
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  private:

    const Eigen::Vector3d transformed_mean_tar_;
  const Eigen::Vector3d transformed_normal_tar_;
  const Eigen::Vector3d src_mean_;
};
...
public:
  ...
  P2DEfficientCost (const Eigen::Vector3d& target_mean, const Eigen::Matrix3d& tar_sqrt_information, const Eigen::Vector3d& src_mean) :
    tar_mean_(target_mean),
  ...
  bool operator()(const T*  Ta,
    ...
    const Eigen::Matrix<T,3,1> trans_mat_src = GetTransMatrix(Ta);
    const Eigen::Matrix<T,3,3> rot_mat_src = GetRotMatrix(Ta);

    //const Eigen::Matrix<T,2,1> transformed_mean_tar = (rot_mat_tar * (tar_mean_).cast<T>()) + trans_mat_tar;
    const Eigen::Matrix<T,3,1> transformed_mean_tar = tar_mean_.cast<T>();
    const Eigen::Matrix<T,3,1> transformed_mean_src = (rot_mat_src * (src_mean_).cast<T>()) + trans_mat_src;
    ...
    residuals.template block<3, 1>(0, 0) = (transformed_mean_src - transformed_mean_tar);
    residuals.applyOnTheLeft(tar_sqrt_information_.template cast<T>());
    return true;
  }

  static ceres::CostFunction* Create(
      const Eigen::Vector3d& target_mean, const Eigen::Matrix3d& tar_sqrt_information, const Eigen::Vector3d& src_mean) {
  ...
  private:
    ...
    const Eigen::Vector3d tar_mean_;
  const Eigen::Matrix3d tar_sqrt_information_;
  const Eigen::Vector3d src_mean_;
};
...
//   P2PCost (const Eigen::Vector3d& target_mean, const Eigen::Vector3d& src_mean) : tar_mean_(target_mean), src_mean_(src_mean) {}

//   template <typename T>
//   bool operator()(const T*  Ta,
//                   const T*  Tb,
//                   T* residuals_ptr) const {
//     const Eigen::Matrix<T,3,1> trans_mat_tar = GetTransMatrix(Ta);
//     const Eigen::Matrix<T,3,3> rot_mat_tar = GetRotMatrix(Ta);

//     const Eigen::Matrix<T,3,1> trans_mat_src = GetTransMatrix(Tb);
//     const Eigen::Matrix<T,3,3> rot_mat_src = GetRotMatrix(Tb);

//     const Eigen::Matrix<T,3,1> transformed_mean_tar = (rot_mat_tar * (tar_mean_).cast<T>()) + trans_mat_tar;
//     const Eigen::Matrix<T,3,1> transformed_mean_src = (rot_mat_src * (src_mean_).cast<T>()) + trans_mat_src;
...
//     residuals_ptr[2] = transformed_mean_tar(2) - transformed_mean_src(2);
//     //residuals_ptr[0] = T(scale_similarity_)*(transformed_mean_src-transformed_mean_tar).norm();
//     return true;
//   }

//   static ceres::CostFunction* Create(
//       const Eigen::Vector3d& target_mean, const Eigen::Vector3d& src_mean) {
//     return new ceres::AutoDiffCostFunction<P2PCost 3, 6>(new P2PCost (target_mean, src_mean));
//   }
//   EIGEN_MAKE_ALIGNED_OPERATOR_NEW
//   private:

//     const Eigen::Vector3d tar_mean_, src_mean_;
...
public:
  ...
  P2PEfficientCost (const Eigen::Vector3d& tar_transformed, const Eigen::Vector3d& src_mean) : tar_transformed_(tar_transformed), src_mean_(src_mean) {}
  ...
  bool operator()(const T*  Tb,
    ...
    const Eigen::Matrix<T,3,1> transformed_mean_tar = tar_transformed_.cast<T>();
    const Eigen::Matrix<T,3,1> transformed_mean_src = (rot_mat_src * (src_mean_).cast<T>()) + trans_mat_src;
    ...
    residuals_ptr[1] = transformed_mean_tar(1) - transformed_mean_src(1);
    residuals_ptr[2] = transformed_mean_tar(2) - transformed_mean_src(2);
    //std::cout <<transformed_mean_tar(0) - transformed_mean_src(0) << residuals_ptr[1] << residuals_ptr[2]<<std::endl;
    //residuals_ptr[0] = T(scale_similarity_)*(transformed_mean_src-transformed_mean_tar).norm();
    return true;
  }
  ...
  static ceres::CostFunction* Create(
      const Eigen::Vector3d& target_mean, const Eigen::Vector3d& src_mean) {
    return new ceres::AutoDiffCostFunction<P2PEfficientCost,3,6>(new P2PEfficientCost (target_mean, src_mean));
  }
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  private:

    const Eigen::Vector3d tar_transformed_, src_mean_;
};

// class P2PEfficientContinuousCost : public RegistrationCost{
// public:

//   P2PEfficientContinuousCost (const Eigen::Vector3d& tar_transformed, const Eigen::Vector3d& src_mean, const double time_factor, const std::vector<double>& velocity) : tar_transformed_(tar_transformed), src_mean_(src_mean), time_factor_(time_factor), velocity_(velocity){}
...
//     const Eigen::Matrix<T,3,1> trans_vel{ ts*velocity_[0], ts*velocity_[1] }; // translation velocityconst Eigen::Matrix<T,2,2> rot_vel = GetRotMatrix2D(vtheta);
...
//   static ceres::CostFunction* Create(
//       const Eigen::Vector3d& target_mean, const Eigen::Vector3d& src_mean, const double time_factor, const std::vector<double>& velocity) {
