...
cell::cell(const pcl::PointCloud<pcl::PointXYZI>::Ptr input, const std::vector<int>& pointIdxNKNSearch, 
  const bool weight_intensity, const Eigen::Vector3d& origin, const double intensity_thre) : Nsamples_(pointIdxNKNSearch.size()) 
{
  // Compute Covariance
  Eigen::Vector3d tot(0, 0, 0);
  Eigen::MatrixXd w(Nsamples_, 1);
  Eigen::MatrixXd x(Nsamples_, 3);
  double min = 8;
  int valid_num =0;
  double max=0;

  for(size_t i=0 ; i<Nsamples_ ; i++){ // build sample and weight block
    x.block<1,3>(i,0) = Eigen::Vector3d(input->points[pointIdxNKNSearch[i]].x, input->points[pointIdxNKNSearch[i]].y, input->points[pointIdxNKNSearch[i]].z);
    w(i,0) = weight_intensity ? std::max(input->points[pointIdxNKNSearch[i]].intensity-min,0.0) : 1.0;
    if(input->points[pointIdxNKNSearch[i]].intensity>max)
       {
  
        max = input->points[pointIdxNKNSearch[i]].intensity;
        tot = x.block<1,3>(i,0).transpose();
      }

    if(input->points[pointIdxNKNSearch[i]].intensity>22)
      valid_num++;
  }
  ...
  {  
    u_.block<3,1>(0,0) += w(i,0)*x.block<1,3>(i,0).transpose();
  }

  for(Eigen::Index i=0 ; i<Nsamples_ ; i++) // subtract mean
    x.block<1,3>(i,0) = x.block<1,3>(i,0) - u_.block<3,1>(0,0).transpose(); //

  Eigen::MatrixXd x_weighted(x.rows(),x.cols()); // Weighted
  for(int i = 0; i < x.rows() ; i++)
    x_weighted.block<1,3>(i,0) = w(i,0)*x.block<1,3>(i,0);

  cov_ = x.transpose()*x_weighted; // weighted
  if(max > intensity_thre)  u_=tot; 
  valid_ = ComputeNormal(origin);
}

bool cell::ComputeNormal(const Eigen::Vector3d& origin)
{
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es(cov_);
  es.compute(cov_);

  snormal_ = es.eigenvectors().col(0);
  orth_normal = es.eigenvectors().col(2);
  lambda_min = es.eigenvalues()[0];
  lambda_max =  es.eigenvalues()[2];
  ...
  const bool cov_reasonable = lambda_min > 0 && lambda_max > 0;
...
MapPointNormal::MapPointNormal(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cld, float radius, const Eigen::Vector3d& origin, const bool weight_intensity, const bool raw, const Eigen::Vector3d& pose) : weight_intensity_(weight_intensity)  {
...
MapPointNormal::MapPointNormal(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cld, float radius, std::vector<cell>& cell_orig, const Eigen::Isometry3d& T){

  input_ = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>());
  downsampled_ = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());
  radius_ = radius;
  pcl::transformPointCloud(*cld, *input_, T.matrix()); // transform to new reference frame

  // const Eigen::Affine3d T3d = Eigen::Translation3d(T.translation().topRows<3>()) *
  //     T.linear().topLeftCorner<3,3>();
  ...
  for(auto && c : cells){
    ...
    pxyz.y = c.u_(1);
    pxyz.z = c.u_(2);
    downsampled_->push_back(pxyz);
  }

  kd_cells.setInputCloud(downsampled_);
}

// void MapPointNormal::Compensate(const Eigen::Isometry3d& Tmot, const bool ccw){
//   EFEAR_4D::Compensate(*input_, Tmot, ccw);
//   std::vector<double> mot_vek;
//   EFEAR_4D::Isometry3dToVectorXYeZ(Tmot, mot_vek);
//   Eigen::Affine2d T(Eigen::Affine2d::Identity());
//   downsampled_->clear();
//   for(size_t i =0 ; i < cells.size() ; i++){
//     double rel_timestamp = GetCellRelTimeStamp(i,ccw);
//     //cout<<i<<", "<<rel_timestamp<<endl;
//     T.linear() = getScaledRotationMatrix(mot_vek, rel_timestamp);
//     T.translation() = getScaledTranslationVector(mot_vek, rel_timestamp);
...
MapNormalPtr MapPointNormal::TransformMap(const Eigen::Isometry3d& T){
...
double MapPointNormal::GetCellRelTimeStamp(const size_t index, const bool ccw){
  ...
  const double z = cells[index].u_(2);
  return EFEAR_4D::GetRelTimeStamp(x, y, ccw);
}
...
void MapPointNormal::ComputeSearchTreeFromCells(){
  ...
  //cout << "downsampled_" << downsampled_->size() <<endl;
...
void MapPointNormal::ComputeNormals(const Eigen::Vector3d& origin){
  ...
  for(int i=0;i<cell_sample_means.size();i++){
    ...
    int density_factor = 5;
    if ( kdt_input.radiusSearchT (pnt, radius_, pointIdxNKNSearch, pointNKNSquaredDistance) >=(radius_*density_factor) ){
      cell c = cell(input_, pointIdxNKNSearch, weight_intensity_, origin);
      if(c.valid_)
        cells.push_back(c);
    }
  }
    // std::cout <<"raw cell size: "<< cell_sample_means.size()
    // << "cells size: " <<cells.size() <<endl;
}
...
Eigen::MatrixXd MapPointNormal::GetCloudTransformed(const Eigen::Isometry3d& Toffset){
...
std::vector<cell> MapPointNormal::TransformCells(const Eigen::Isometry3d& T){
...
void MapPointNormal::Transform(const Eigen::Isometry3d& T, Eigen::MatrixXd& means, Eigen::MatrixXd& normals){
  means = GetMeans();
  normals = GetNormals();
  for(int i=0;i<means.cols();i++)
    means.block<3,1>(0,i) = T*means.block<3,1>(0,i);
...
visualization_msgs::MarkerArray Cells2Markers(std::vector<cell>& cells, const ros::Time& time, const std::string& frame, int val, float alpha){
  ...
  for(int i=0 ; i<cells.size() ; i++){
    ...
    Eigen::Vector3d end = u + cells[i].snormal_*5;//d
    geometry_msgs::Point p;
    //Eigen::Vector3d trans = cells[i].snormal_*d;
    //Eigen::Quaterniond quat(trans);
...
cell cell::TransformCopy(const Eigen::Isometry3d& T){
  const Eigen::Matrix3d R = T.linear();
  const Eigen::Matrix3d C = R*T*cov_*R.transpose();
  const Eigen::Vector3d N = R*snormal_;
  const Eigen::Vector3d N_orth = R*orth_normal;
  const Eigen::Vector3d U = T*u_;
...
//   return alpha;
// }
void MapPointNormal::PublishDataAssociationsMap(const ros::Publisher& publisher, const std::vector<std::tuple<Eigen::Vector3d,Eigen::Vector3d,double,int> >& vis_residuals){
  ...
  def_marker.scale.z = 0.1;
  def_marker.header.frame_id = "baselink";
...
void MapPointNormal::PublishMap(const ros::Publisher& publisher, const MapNormalPtr map,const std::string& frame_id, const int value, float alpha){
  if(map==NULL)
    return;
