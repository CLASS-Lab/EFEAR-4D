...
void Compensate(pcl::PointCloud<pcl::PointXYZI>& cloud, const Eigen::Affine3d& Tmotion, bool ccw){
  ...
  //EFEAR_4D::Affine3dToVectorXYeZ(Tmotion, mot);
  //Compensate(cloud, mot, ccw);
}

void Isometry3dToVectorXYeZ(const Eigen::Isometry3d& T, std::vector<double>& par) {
  if(par.size()!=6)
    par.resize(6,0);
  Eigen::Matrix3d Rotation = T.rotation();
  Eigen::Vector3d eul = Rotation.eulerAngles(2,1,0);
  par[0] = T.translation()(0);
  par[1] = T.translation()(1);
  par[2] = T.translation()(2);
  par[3] = eul(0);//R Z
  par[4] = eul(1);//P Y
  par[5] = eul(2);//Y X
  // par.push_back(T.translation()(0));
  // par.push_back(T.translation()(1));
  // par.push_back(T.translation()(2));
  //cout << "par: " << par[0] << endl;
}

void Isometry3dToEigVectorXYeZ(const Eigen::Isometry3d& T, Vector6d& par) {
  Eigen::Matrix3d Rotation = T.rotation(); // 直接从Isometry3d获取旋转矩阵
  Eigen::Vector3d eul = Rotation.eulerAngles(2,1,0);
  par << T.translation()(0), T.translation()(1), T.translation()(2), eul(0), eul(1), eul(2);
  //cout << "par: " << par(0) << endl;
}
