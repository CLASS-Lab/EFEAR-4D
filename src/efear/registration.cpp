...
double Registration::Weights::GetWeight(const weightoption opt){
  ...
  // case weightoption::Sim_intensity:    return Similarity(plan1, plan2);
...
Eigen::Isometry3d vectorToIsometry3d(const std::vector<double>& vek){
  assert(vek.size() == 6);
  Eigen::AngleAxisd rot_x_btol(vek[5], Eigen::Vector3d::UnitX());
  Eigen::AngleAxisd rot_y_btol(vek[4], Eigen::Vector3d::UnitY());
  Eigen::AngleAxisd rot_z_btol(vek[3], Eigen::Vector3d::UnitZ());
  // cout <<"vek: "<< (Eigen::Translation3f(vek[0], vek[1], vek[2]) *
  //     rot_z_btol * rot_y_btol * rot_x_btol).matrix()<<endl;
  return (Eigen::Translation3d(vek[0], vek[1], vek[2]) *
      rot_z_btol * rot_y_btol * rot_x_btol);
}

Eigen::Isometry3d vectorToIsometry3d(double x, double y, double z, double ez, double ey, double ex) {

  return Eigen::Translation<double, 3>(x, y, z) *
      Eigen::AngleAxis<double>(ez, Eigen::Vector3d::UnitX()) *
      Eigen::AngleAxis<double>(ey, Eigen::Vector3d::UnitY()) *
      Eigen::AngleAxis<double>(ex, Eigen::Vector3d::UnitZ());
}

Eigen::Affine2d  vectorToAffine2d(double x, double y, double ez) {
  Eigen::Affine3d T = vectorToIsometry3d(x, y, 0, 0, 0, ez);
